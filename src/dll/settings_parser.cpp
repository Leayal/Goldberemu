...
uint32 create_localstorage_settings(Settings **settings_client_out, Settings **settings_server_out, Local_Storage **local_storage_out)
{
...

    {
        ...
        if (Local_Storage::get_file_data(program_path + "local_save.txt", array, sizeof(array) - 1) != -1) {
            save_path = program_path + Settings::sanitizeA(array);
        }
    }
    ...
    // Acount name
    wchar_t name[32] = {};
    if (local_storage->get_wstr_settings("account_name.txt", name, sizeof(name) - 1) <= 0) {
        wcscpy(name, DEFAULT_NAME);
        local_storage->store_wstr_settings("account_name.txt", name, wcslen(name));
    }
    ...
    bool disable_overlay = false;
    {
        ...
        for (auto & p: paths) {
            ...
            if (p == "offline.txt") {
                ...
                int len = Local_Storage::get_file_wstr(steam_settings_path + "force_account_name.txt", name, sizeof(name) - 1);
                if (len > 0) name[len] = 0;
